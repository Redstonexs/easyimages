...
- Runtime images live under `i/`; runtime logs and image metadata live under `admin/logs/`; frontend build output is `public/dist/`.
...
- Focused Go tests: `go test ./internal/service -run TestName`, `go test ./internal/handler -run TestName`, or `go test . -run TestName` for root tests.
- Static check: `go vet ./...`.
...
- Do not commit local runtime files: `config/config.json`, `config/config.guest.json`, `config/api_key.json`, `config/install.lock`, `config/EasyImage.lock`, `config/php_backup/`, `admin/logs/`, or files under `i/` except `i/.gitkeep`.
...
- `internal/handler/admin_api.go` contains Vue admin JSON APIs under `/admin/api/*` and exposes secret presence flags instead of external captcha secret values.
...
- Default config values live in `config/config.go`; notable defaults are `Path=/i/`, `StoragePath=Y/m/d/`, `Port=8080`, `ListNumber=20`, and `ListDate=10`.
...
- Uploads call `service.ProcessUpload`, which validates size/extension, stores under expanded `cfg.StoragePath`, queues post-processing, and records metadata.
